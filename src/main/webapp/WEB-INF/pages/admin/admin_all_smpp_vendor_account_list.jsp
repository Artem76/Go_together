...
<head>
    ...
    <!-- Title -->
    <title>${title} - SMPP Vendor Accounts</title>
...
<div class="wrapper">
    ...
    <li class="nav-item dropdown">
        <a href="/admin_full_smpp_vendor_account_edit?id_vendor_account=0&id_customer=0&error=&err_name=&err_ips=&return_in_customer=no" class="nav-link">
...
<script>
    ...
    smppVendorAccountList = [];
    <c:forEach var="smppVendorAccount" items="${smppVendorAccountList}">
    newAccount = {};
    newAccount['idCustomer'] = ${smppVendorAccount.customer.id};
    newAccount['idAccount'] = ${smppVendorAccount.id};
    newAccount['customer'] = '${smppVendorAccount.customer.name}';
    newAccount['account'] = '${smppVendorAccount.name}';
    newAccount['systemId'] = '<c:forEach var="smppVendorIps" items="${smppVendorAccount.smppVendorIpsList}" varStatus="vs">${smppVendorIps.systemId}<c:if test="${not vs.last}">, </c:if></c:forEach>';
    smppVendorAccountList.push(newAccount);
    </c:forEach>
...
<script type="text/javascript" src="/js/_all_smpp_vendor_account_list.js"></script>
