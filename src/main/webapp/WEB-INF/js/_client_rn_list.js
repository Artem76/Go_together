...
$(document).ready(function () {
    ...
    if (document.getElementById('jsGrid-client-rn-list')) {
        ...
        window.jsGrid_init = function () {
            $("#jsGrid-client-rn-list").jsGrid({
                ...
                rowClick: function(args) {
                    if (args.item.date == args.event.target.innerHTML || args.item.smppClientAccount == args.event.target.innerHTML || args.item.user == args.event.target.innerHTML){
                        document.location.href = "/admin_full_client_rn_edit?id=" + args.item.id;
                    }
                },
                fields: [
                    {title: "Upload date", name: "date", type: "text", width: 100, align: "center"},
                    {title: "Customer - account", name: "smppClientAccount", type: "text", width: 200, align: "center"},
                    {title: "User", name: "user", type: "text", width: 100, align: "center"},
                    {title: "Related emails", name: "emails", type: "text", width: 200, align: "center"},
    ...
    $("#refresh_list").click(function () {
        ...
        document.account_id = $("#client_menu").val();
    ...
    function refreshList() {
        loadIndicator.show();
        document.rn = [];
        $.post('/admin_full_client_rn_list',{
            ...
            for (var i = 0, len = rates.length; i < len; i++) {
                var o2 = {};
                o2['id'] = rates[i]["id"];
                o2['smppClientAccount'] = rates[i]["smppClientAccount"];
                o2['date'] = rates[i]["date"];
                o2['user'] = rates[i]["user"];
                o2['emails'] = rates[i]["emails"];
