...
@Service
public class StatsUpdateTaskLongPeriod {
    ...
    public void runParallelStream(){
        ...
        loggerService.writeInfo("Long period stats update task started");
        ...
        String query = "SELECT date_trunc('hour', created_at) as created_at, softswitch_id, uid, routed_cid, mcc, mnc, delivery_status as status, SUM(delivery_time) as delivery_time, SUM(1) as count\n" +
                "FROM mdr WHERE created_at BETWEEN date_trunc('hour', current_timestamp at time zone 'UTC' - Interval '2 day') AND date_trunc('hour', current_timestamp at time zone 'UTC' - Interval '240 minute') AND (NOT(delivery_status IS NULL))\n" +
        ...
        try {
            ...
            loggerService.writeInfo("Long period stats update task finished without errors. Updated " + totalsRowsCount + " rows of cache.");
            ...
            loggerService.writeInfo("Long period stats update task finished with errors. Error : \n" + e.toString());
